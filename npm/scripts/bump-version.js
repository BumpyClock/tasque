...
const { platformPackageManifests } = require("./postinstall");
...
const PLATFORM_PKGS = platformPackageManifests().map(
...
function main() {
	...
	for (const update of updates) {
		...
		console.log(
			`${path.relative(process.cwd(), update.filePath)}: ${update.prev} -> ${update.next}`,
		);
	}
