...
pub(crate) fn apply_task_created(
    ...
    let alias = match as_string(payload.get("alias")) {
        Some(alias) if alias.eq_ignore_ascii_case(&event.task_id) => {
            return Err(invalid_created_alias_error(event, alias));
        }
        Some(alias) => normalize_alias(&alias),
...
fn invalid_created_alias_error(event: &EventRecord, alias: String) -> TsqError {
    TsqError::new(
        "INVALID_EVENT",
