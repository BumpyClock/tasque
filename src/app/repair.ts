...
export async function executeRepair(
  ...
  if (!opts.fix) {
    // Dry-run: compute plan outside lock (no writes)
  ...
  // Pre-lock: handle force-unlock (must happen before acquiring the lock)
  if (opts.forceUnlock) {
    const hasLock = await lockExists(repoRoot);
    if (hasLock) {
  ...
  return withWriteLock(repoRoot, async () => {
    // Compute plan inside the lock using the locked snapshot
    const { state: lockedState, allEvents } = await loadProjectedState(repoRoot);
    const { orphaned_deps, orphaned_links } = buildRepairPlan(lockedState);
    const { stale_temps, stale_lock, old_snapshots } = await scanFilesystem(repoRoot);
    const plan: RepairPlan = {
      orphaned_deps,
      orphaned_links,
    ...
    for (const link of plan.orphaned_links) {
      events.push(
        makeEvent(actor, now(), "link.removed", link.src, {
          type: link.type,
          target: link.dst,
        }),
      );
